...
for i in range (X_input_gray.shape[0]):
    for j in range (X_input_gray.shape[1]):
        for k in range (X_input_gray.shape[2]):
            rgb = X_input_gray[i,j,k]
            gray = rgb2gray(rgb)
            X_input_gray[i,j,k] = gray

numberimages = 1
X_label = np.empty((numberimages * 1 * 1,)
X_label.fill(numberimages)
...
with tf.Session() as sess:
    loader = tf.train.import_meta_graph('~/model/model.ckpt-232000.meta')
    loader.restore(sess, tf.train.latest_checkpoint('~/model'))
    #evaluate(X_test,y_test)
    predict_results = predict(X_test,y_test)
